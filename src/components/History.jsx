const History = () => {
	return (
		<div className="py-10 mx-5">
			<h3 className="font-semibold uppercase text-resalted-50 md:px-16 py-4 rounded-md text-3xl md:text-5xl my-12 bg-backblue-200 shadow-lg text-center md:w-1/2 mx-auto">
				Conocenos
			</h3>
			<div className="max-w-screen-lg flex flex-col md:flex-row md:gap-5 mx-auto justify-center items-center">
				<div className="w-full">
					<img
						src="https://img.freepik.com/foto-gratis/gente-negocios-reunion_53876-94720.jpg?w=740&t=st=1684873295~exp=1684873895~hmac=06762ba1ae482761558c7e170efb2783594c966f94b969aa6a898740a9565bc2"
						alt=""
						className="grayscale rounded-md"
					/>
				</div>
				<div className=" text-white bg-backblue-100 p-6 mx-2 rounded-md transform -translate-y-28 md:transform-none shadow-md">
					<p>
						Desde su establecimiento en 1995, nuestro estudio de
						abogados se ha consolidado como una reconocida firma
						legal comprometida con la excelencia y la defensa de los
						derechos de nuestros clientes. A lo largo de los años,
						hemos construido una sólida reputación basada en la
						experiencia, la dedicación y los resultados exitosos que
						hemos obtenido en diversas áreas del derecho.
					</p>
					<p>
						Desde nuestros inicios, nos hemos esforzado por brindar
						un servicio legal integral y personalizado, adaptado a
						las necesidades y circunstancias únicas de cada cliente.
						Nuestro equipo de abogados altamente capacitados y
						comprometidos ha trabajado incansablemente para obtener
						los mejores resultados legales para nuestros clientes,
						ya sea en casos complejos de derecho penal, disputas
						civiles o asuntos laborales.
					</p>
					<p>
						A lo largo de los años, hemos construido relaciones
						sólidas con nuestros clientes, basadas en la confianza,
						la confidencialidad y la comunicación abierta. Nos
						enorgullece haber ayudado a numerosos individuos,
						familias y empresas a resolver sus problemas legales de
						manera efectiva, brindando representación sólida y
						estratégica en cada etapa del proceso legal.
					</p>
				</div>
			</div>
		</div>
	);
};

export default History;
